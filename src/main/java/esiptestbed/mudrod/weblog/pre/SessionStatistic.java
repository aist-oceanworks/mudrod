...
public class SessionStatistic extends DiscoveryStepAbstract{
	...
	public void processSession() throws IOException, InterruptedException, ExecutionException {
		...
		SearchResponse sr = es.client.prepareSearch(config.get("indexName")).setTypes(inputType)
		...
		for (Terms.Bucket entry : Sessions.getBuckets()) {
			if (entry.getDocCount() >= 3 && !entry.getKey().equals("invalid")) {
				...
				if (searchDataListRequest_count != 0 && searchDataListRequest_count <=Integer.parseInt(config.get("searchf")) && searchDataRequest_count != 0 && searchDataRequest_count <= Integer.parseInt(config.get("viewf")) && ftpRequest_count <= Integer.parseInt(config.get("downloadf"))) {
					...
					IndexRequest ir = new IndexRequest(config.get("indexName"), outputType).source(jsonBuilder().startObject()
