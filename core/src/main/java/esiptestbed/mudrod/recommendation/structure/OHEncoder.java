...
public class OHEncoder {
		...
		while (true) {
			for (SearchHit hit : scrollResp.getHits().getHits()) {
				...
				try {
					...
					UpdateRequest ur = es.genUpdateRequest(indexName, metadataType, hit.getId(), metadatacode);
	...
	private Map<String, Object> OHEncodeMetadata(ESDriver es, Map<String, Object> metadata)
		...
		for (int i = 0; i < CategoryNum; i++) {
			...
			if (metadata.get(var) != null && metadata.get(var) != "") {
				String value = es.customAnalyzing(indexName, "csv", metadata.get(var).toString());
			...
			for (int k = 0; k < codeArr1.length; k++) {
				codeArr[k] = codeArr[k] * CategoricalVarWeights.get(var);
			}*/
	...
	private Map<String, Vector> OHEncodeVar(ESDriver es, String varName) {
		...
		SearchResponse sr = es.client.prepareSearch(indexName).setTypes(metadataType)
