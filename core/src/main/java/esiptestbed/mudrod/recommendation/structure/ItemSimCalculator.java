...
public class ItemSimCalculator implements Serializable {
	...
	public JavaPairRDD<String, List<String>> prepareData(JavaSparkContext sc, String path) {
		...
		JavaPairRDD<String, List<String>> testRDD = useritem_rateRDD
				.flatMapToPair(new PairFlatMapFunction<Tuple2<String, Integer>, String, List<String>>() {
					@Override
					public Iterable<Tuple2<String, List<String>>> call(Tuple2<String, Integer> arg0) throws Exception {
						...
						for (int i = 0; i < n; i++) {
							...
							l.add(words.split(",")[1]);
							Tuple2<String, List<String>> worddoc = new Tuple2<String, List<String>>(words.split(",")[0],
									l);
							pairs.add(worddoc);
						}
						return pairs;
	...
	public JavaPairRDD<String, List<String>> filterData(ESDriver es,
		...
		JavaPairRDD<String, List<String>> filterUserDatasetsRDD = userDatasetsRDD
				.mapToPair(new PairFunction<Tuple2<String, List<String>>, String, List<String>>() {
					public Tuple2<String, Integer> call(String s) {
						String[] sarray = s.split(",");
						Double rate = Double.parseDouble(sarray[2]);
						return new Tuple2<String, Integer>(sarray[0] + "," + sarray[1], rate.intValue());
	...
	public List<LinkageTriple> CalItemSimfromMatrix(SparkDriver spark, String CSV_fileName, int skipRow) {

		JavaPairRDD<String, Vector> importRDD = MatrixUtil.loadVectorFromCSV(spark, CSV_fileName, skipRow);

		IndexedRowMatrix indexedMatrix = MatrixUtil.buildIndexRowMatrix(importRDD.values());
		...
		JavaRDD<LinkageTriple> tripleRDD = cartesianRDD
				.map(new Function<Tuple2<Tuple2<String, Vector>, Tuple2<String, Vector>>, LinkageTriple>() {
